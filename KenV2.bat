...
set "ZIP_PAYLOAD=%TARGET%\KenV2 Tools.zip"
...
echo %CYAN%Prüfe auf Updates...%RESET%

for /f "usebackq tokens=*" %%v in (`powershell -Command "(Invoke-WebRequest -Uri '%UPDATE_URL%' -UseBasicParsing).Content.Trim()"`) do set "REMOTE_VERSION=%%v"

if not defined REMOTE_VERSION (
    echo %YELLOW%Update-Check fehlgeschlagen.%RESET%
    timeout /t 1 >nul
    exit /b
)

if "%REMOTE_VERSION%"=="%VERSION%" (
    echo %GREEN%KenV2 ist aktuell (%VERSION%).%RESET%
    timeout /t 1 >nul
    exit /b
)

echo.
echo %YELLOW%Neue Version verfuegbar: %REMOTE_VERSION%%RESET%
choice /c JN /m "Jetzt aktualisieren?"

if errorlevel 2 exit /b

echo %CYAN%Lade neue Version herunter...%RESET%
curl -L -o "%TEMP%\KenV2_new.bat" "https://raw.githubusercontent.com/27migu/KenV2/main/KenV2.bat" --silent

if not exist "%TEMP%\KenV2_new.bat" (
    echo %RED%Download fehlgeschlagen.%RESET%
    timeout /t 2 >nul
    exit /b
)

echo %CYAN%Installiere Update...%RESET%

(
echo @echo off
echo timeout /t 2 ^>nul
echo del "%~f0"
echo copy "%TEMP%\KenV2_new.bat" "%~f0" ^>nul
echo start "" "%~f0"
echo del "%%~f0"
) > "%TEMP%\KenV2_updater.bat"

start "" "%TEMP%\KenV2_updater.bat"
exit
...
:: ==========================
:: UPDATE CHECK + TOOLS DOWNLOAD
:: ==========================
echo %CYAN%Prüfe auf neue Version...%RESET%
set "LOCAL_VERSION=%VERSION%"
set "REMOTE_VERSION="

:: Remote Version auslesen
for /f "usebackq tokens=*" %%v in (`powershell -Command "(Invoke-WebRequest -Uri '%UPDATE_URL%' -UseBasicParsing).Content.Trim()"`) do set "REMOTE_VERSION=%%v"

if not defined REMOTE_VERSION (
    echo %YELLOW%Update Check fehlgeschlagen, Internetverbindung prüfen.%RESET%
) else (
    echo Lokale Version: %LOCAL_VERSION%
    echo Verfügbare Version: %REMOTE_VERSION%

    :: Versionsvergleich
    powershell -Command "exit ([version]('%REMOTE_VERSION%') -gt [version]('%LOCAL_VERSION%'))"
    if %errorlevel%==0 (
        echo %GREEN%KenV2 ist aktuell.%RESET%
    ) else (
        echo %YELLOW%Neue Version gefunden! Lade herunter...%RESET%
        set "NEW_ZIP=%TARGET%\KenV2_v%REMOTE_VERSION%.zip"
        curl -L -o "%NEW_ZIP%" "%GITHUB_REPO%/%REMOTE_VERSION%/KenV2.zip" --progress-bar
        echo %GREEN%Neue Version %REMOTE_VERSION% heruntergeladen.%RESET%
    )
)

:: ==========================
:: TOOLS DOWNLOAD (immer aktuell)
:: ==========================
echo %CYAN%Lade KenV2 Tools herunter...%RESET%
curl -L -o "%TOOLS_DIR%\KenV2 Tools.zip" "%GITHUB_REPO%/%REMOTE_VERSION%/KenV2 Tools.zip" --progress-bar
echo %GREEN%KenV2 Tools heruntergeladen und bereit.%RESET%
...
echo ───────────────────────────────────────────────
echo [1-6] Option   [N] Next   [P] Previous   [Q] Quit   [H] Hardware
...
cls
call :printHeader
echo %CYAN%===== Windows Debloat =====%RESET%
echo.
echo   [R] Restore Point erstellen
echo   [1] Telemetrie u Tracking deaktivieren
...
echo  [17] Game DVR u. Game Mode deaktivieren
...
:: --- Zahlen prüfen ---
if /i "%DEB_CHOICE%"=="1" goto Telemetrie
if /i "%DEB_CHOICE%"=="2" goto Cortana
if /i "%DEB_CHOICE%"=="3" goto OneDrive
if /i "%DEB_CHOICE%"=="4" goto Apps
if /i "%DEB_CHOICE%"=="5" goto Files
if /i "%DEB_CHOICE%"=="6" goto ActivityHistory
if /i "%DEB_CHOICE%"=="7" goto ConsumerFeatures
if /i "%DEB_CHOICE%"=="8" goto Hibernation
if /i "%DEB_CHOICE%"=="9" goto LocationTracking
if /i "%DEB_CHOICE%"=="10" goto Biometrics
if /i "%DEB_CHOICE%"=="11" goto Widgets
if /i "%DEB_CHOICE%"=="12" goto PowerShellTelemetrie
if /i "%DEB_CHOICE%"=="13" goto BrowserDebloat
if /i "%DEB_CHOICE%"=="14" goto FSE
if /i "%DEB_CHOICE%"=="15" goto Netzwerk1
if /i "%DEB_CHOICE%"=="16" goto Edge
if /i "%DEB_CHOICE%"=="17" goto DVR
if /i "%DEB_CHOICE%"=="18" goto UserTracking

:: --- Buchstaben prüfen ---
if /i "%DEB_CHOICE%"=="B" goto page1
if /i "%DEB_CHOICE%"=="R" goto createRestore
...
:Telemetrie
echo Deaktiviere Telemetrie u. Tracking...
...
echo Deaktiviere ausgewaehlte Dienste...
echo.
...
:Files
echo Temp / Log / Old Files löschen...
rd /s /q "%TEMP%" >nul 2>&1
...
:Edge
echo MC Edge u. Home aus Explorer entfernen...
...
:DVR
echo Game DVR u. Game Mode deaktivieren...
...
:UserTracking
echo User Tracking komplett deaktivieren...
...
:: --- Zahlen prüfen ---
if /i "%BIOS_CHOICE%"=="1" goto bios1
if /i "%BIOS_CHOICE%"=="2" goto bios2
if /i "%BIOS_CHOICE%"=="3" goto bios3
if /i "%BIOS_CHOICE%"=="4" goto bios4
...
echo   ╚══════════════╝  ╚══════════════╝  ╚══════════════╝  ╚══════════════╝
echo.
echo %WHITE%[N] Next Page   [P] Previous Page  %RESET%
echo.
...
if /i "%input%"=="1" goto hardwareCPU
if /i "%input%"=="2" goto hardwareGPU
if /i "%input%"=="3" goto hardwareRAM
if /i "%input%"=="4" goto hardwareStorage
if /i "%input%"=="N" goto page3
if /i "%input%"=="P" goto page1
...
if /i "%input%"=="1" (
    if /I "!CPU_TYPE!"=="Intel" (
        echo Intel CPU Tweaks werden ausgeführt...
bcdedit /set hyperthreading off
sc stop "DiagTrack" >nul 2>&1
sc config "DiagTrack" start=disabled >nul 2>&1
sc stop "dmwappushservice" >nul 2>&1
sc config "dmwappushservice" start=disabled >nul 2>&1
reg add "HKLM\SOFTWARE\Policies\Intel" /v DisableTelemetry /t REG_DWORD /d 1 /f >nul 2>&1
reg add "HKLM\SYSTEM\CurrentControlSet\Control\Power\PowerSettings\54533251-82be-4824-96c1-47b60b740d00\5d76a2ca-e8c0-402f-a133-2158492d58ad" /v Attributes /t REG_DWORD /d 0 /f
    ...
    if /I "!CPU_TYPE!"=="AMD" (
        echo AMD CPU Tweaks werden ausgeführt...
reg add "HKLM\SYSTEM\CurrentControlSet\Control\Power\PowerSettings\54533251-82be-4824-96c1-47b60b740d00\5d76a2ca-e8c0-402f-a133-2158492d58ad" /v Attributes /t REG_DWORD /d 0 /f
sc stop "AMDRyzenMasterService" >nul 2>&1
sc config "AMDRyzenMasterService" start=disabled >nul 2>&1
    )
    pause
    goto hardwareCPU
)
...
if /i "%input%"=="2" (
    echo Allgemeine CPU Tweaks werden ausgeführt...
dism /Online /Disable-Feature:Microsoft-Hyper-V-All /Quiet /NoRestart
...
:: errorlevel muss absteigend geprüft werden
if errorlevel 6 goto page2   :: B gedrückt
...
:: Errorlevel absteigend prüfen
if errorlevel 7 goto page2    :: B gedrückt
...
echo   [3] USB Tweaks
echo.
echo %GRAY%[P] Previous [N] Next %RESET%
choice /c 123PN /n /m "Auswahl: "

if errorlevel 5 goto page4
if errorlevel 4 goto page2
...
echo.
echo %GRAY%[P] Previous [N] Next%RESET%
choice /c 12PN /n /m "Auswahl: "

if errorlevel 4 goto page5
if errorlevel 3 goto page3
...
call :printHeader
echo.
echo %CYAN%============= Chrome u. Discord =============%RESET%
echo.
...
echo.
echo %GRAY%[P] Previous [N] Next%RESET%
choice /c 12345PN /n /m "Auswahl: "

if errorlevel 7 goto page6
if errorlevel 6 goto page4
...
echo   [3] VPN u. Proxy deaktivieren
...
if /i "%input%"=="1" goto lowLatency
if /i "%input%"=="2" goto highThroughput
if /i "%input%"=="3" goto vpnProxy
if /i "%input%"=="4" goto netbios
if /i "%input%"=="5" goto meteredConn
if /i "%input%"=="6" goto dnsSettings
if /i "%input%"=="7" goto netShares
if /i "%input%"=="8" goto usageStats
if /i "%input%"=="9" goto energyOpts
if /i "%input%"=="A" goto ipv6Teredo
if /i "%input%"=="P" goto page5
...
for /f "tokens=2 delims==" %%a in ('wmic nic where "NetEnabled=true" get Name /value ^| find "="') do (
    ...
    :: Offload Deaktivieren (IPv4 u. IPv6)
...
echo Netzwerkkennung u. Freigaben deaktiviert.
pause
goto page6
...
:: ==========================
:: HANDLE PAGE 1 - Windows & System Tweaks
:: ==========================
:handlePage1
:: Windows Tweaks Auswahl
if /i "%CHOICE%"=="1" (
    goto windowsDebloat
)
if /i "%CHOICE%"=="2" (
...
:: ==========================
:: HANDLE PAGE 2 - CPU / RAM / GPU Tweaks
:: ==========================
:handlePage2
set "CHOICE=%~1"
if "%CHOICE%"=="1" goto cpuTweaks
if "%CHOICE%"=="2" goto ramTweaks
if "%CHOICE%"=="3" goto gpuTweaks
if /i "%CHOICE%"=="B" goto menuStart
goto menuStart

:: ==========================
:: HANDLE PAGE 3 - Storage Tweaks
:: ==========================
:handlePage3
set "CHOICE=%~1"
if "%CHOICE%"=="1" goto diskCache
if "%CHOICE%"=="2" goto defragOptions
if "%CHOICE%"=="3" goto trimOptimize
if /i "%CHOICE%"=="B" goto menuStart
goto menuStart

:: ==========================
:: HANDLE PAGE 4 - Windows Tweaks & Privacy
:: ==========================
:handlePage4
set "CHOICE=%~1"
if "%CHOICE%"=="1" goto telemetryOff
if "%CHOICE%"=="2" goto cortanaDisable
if "%CHOICE%"=="3" goto userTrackingOff
if "%CHOICE%"=="4" goto cleanLogs
if /i "%CHOICE%"=="B" goto menuStart
goto menuStart

:: ==========================
:: HANDLE PAGE 5 - Power / Fortnite / Misc Tweaks
:: ==========================
:handlePage5
set "CHOICE=%~1"
if "%CHOICE%"=="1" goto powerTweaks
if "%CHOICE%"=="2" goto gamemodeTweaks
if "%CHOICE%"=="3" goto gameDvrTweaks
if "%CHOICE%"=="4" goto mkbTweaks
if "%CHOICE%"=="5" goto extrasTweaks
if /i "%CHOICE%"=="B" goto menuStart
goto menuStart

:: ==========================
:: HANDLE PAGE 6 - Network Tweaks
:: ==========================
:handlePage6
set "CHOICE=%~1"
if "%CHOICE%"=="1" goto vpnProxy
if "%CHOICE%"=="2" goto netbios
if "%CHOICE%"=="3" goto meteredConnection
if "%CHOICE%"=="4" goto dnsHighPerf
if "%CHOICE%"=="5" goto networkDiscovery
if "%CHOICE%"=="6" goto clearStats
if "%CHOICE%"=="7" goto networkPower
if "%CHOICE%"=="8" goto tcpIpTweaks
if "%CHOICE%"=="9" goto nicTweaks
if /i "%CHOICE%"=="A" goto ipv6Teredo
if /i "%CHOICE%"=="Z" goto advancedNetwork
if /i "%CHOICE%"=="B" goto menuStart
goto menuStart
